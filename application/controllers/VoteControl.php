...
class VoteControl extends CI_Controller
{
    ...
    public function newRow()
    {
        $keys="phone";
        $this->hedVerify($keys);
        $resultNum = $this->vote->addData($this->dataArr, $this->userArr['Mobile']);
	...
	{
		$keys="pages,rows,competition_id,powerdept,DeptId,DataScope";
		$this->hedVerify($keys);
		$result = $this->vote->getVote($this->dataArr);
		if (count($result) >= 0) {
     ...
     * DateTime: 2021/6/18 10:51
     */
    public function showsignRow()
    {
        $keys="pages,rows,competition_id,DeptId";
        $this->hedVerify($keys);
        $result = $this->vote->showsign($this->dataArr);
        if (count($result) >= 0) {
            $resulArr = build_resulArr('D000', true, '获取成功', json_encode($result));
            http_data(200, $resulArr, $this);
        } else {
