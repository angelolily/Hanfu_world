...
//新增课程
    ...
    public function getimagedetail($pic){
        ...
        if($handler){
            ...
            $arrdirfiles=array();
            $dirfilename = "https://hftx.fzz.cn" . $dir_original .'/'. $pic['course_graphic'] ;
            //5、关闭目录
            closedir($handler);
    ...
    public function getimagecover($pic){
        ...
        if($handler){
            $dir_original=str_replace('.','',$dir_original);
            $dirfilename = "https://hftx.fzz.cn" . $dir_original .'/'. $pic['course_cover'];
            //5、关闭目录
            closedir($handler);
...
//获取
    public function getcourse($searchWhere = []) //查询到赛事表
    {
            $where = "";
            if (count($searchWhere) > 0) {
                if ($searchWhere['course_id'] != '') {//赛事ID  下拉
